...
var genieMotion = new Motion(updateGenie);
var meshes = {};  
...
function setup_render_main() {
    var F_left = 0.0, F_width=1.0, F_top = 0.0; F_height=1.0;
    var left = Math.floor( window.innerWidth * F_left );
    var top = Math.floor( window.innerHeight * F_top );
    var width = Math.floor( window.innerWidth * F_width );
    var height = Math.floor( window.innerHeight * F_height );
...
function update() {
    ...
    if (sceneName=="Luxo" && animation && meshesLoaded && aniTime < 19.4){
        luxoMotion.timestep(aniDt); 
        timeBar.position.set(-10+4*luxoMotion.currTime,0,0);    // advance time-bar position in spline window
    }
    if (sceneName=="Luxo" && animation && meshesLoaded && aniTime > 20 && aniTime < 23.85) {   
        meshes["teapot1"].rotation.y += Math.PI / 32;
    }
    if (sceneName=="Luxo" && animation && meshesLoaded && aniTime > 25) {     
	//pencilMotion.timestep(aniDt);      // advance animation 
    genieMotion.timestep(aniDt);
